...
# note: unsigned macOS .tar.gz may fail Gatekeeper; prefer signed DMGs below
...
# optional linux/mac product archives when present
if [ -f ../product/target/products/ApacheDirectoryStudio-${VERSION}-linux.gtk.aarch64.tar.gz ]; then
  cp ../product/target/products/ApacheDirectoryStudio-${VERSION}-linux.gtk.aarch64.tar.gz ${DIST_DIR}/
fi
if [ -f ../product/target/products/ApacheDirectoryStudio-${VERSION}-macosx.cocoa.aarch64.tar.gz ]; then
  cp ../product/target/products/ApacheDirectoryStudio-${VERSION}-macosx.cocoa.aarch64.tar.gz ${DIST_DIR}/
fi
if [ -f ../product/target/products/ApacheDirectoryStudio-${VERSION}-macosx.cocoa.x86_64.tar.gz ]; then
...
# macOS DMGs (Intel and/or Apple Silicon)
for dmg in ../installers/macos/target/ApacheDirectoryStudio-${VERSION}-macosx.cocoa.*.dmg; do
  if [ -f "$dmg" ]; then
    cp "$dmg" ${DIST_DIR}/
  fi
done
